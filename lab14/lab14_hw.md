...
The neutral alignment has a long distribution.
...
The normalized beach bugs data is more informative because you can see the differences in color for the normalized bug level in each site.  
...
```r
questions_list <- list(evil_vec, eye_vec, male_vec, bald_vec)
```
...
```r
my_list<- list(female_vec, blond_hair_vec, good_vec)
```


```r
ggVennDiagram(my_list, category.names = c("Female", "Blond Hair", "Good"))
```
...
ggplot(aes(
  ...
  theme_minimal()+
  labs(title = "Common Superhero Powers")
```
...
ggplot(aes(
  ...
  theme_minimal()+
  labs(title = "Powerful Superheros")
```
